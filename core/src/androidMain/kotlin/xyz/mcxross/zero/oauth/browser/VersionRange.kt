...
/**
 * A browser filter which matches when a browser falls into a version range. Versions are expected
 * to match the semantics of [DelimitedVersion].
 */
class VersionRange
/**
 * Creates a version range with the specified bounds. A null bound is treated as "no bound" on that
 * end.
 */
(private val mLowerBound: DelimitedVersion?, private val mUpperBound: DelimitedVersion?) {
  /**
   * Determines whether the specified version (parsed as an [DelimitedVersion] falls within the
   * version range.
   */
  ...
  companion object {
    ...
    /**
     * Creates a version range that will match any version at or above the specified version, which
     * will be parsed as a [DelimitedVersion].
     */
    ...
    /**
     * Creates a version range that will match any version at or below the specified version, which
     * will be parsed as a [DelimitedVersion].
     */
    ...
    fun between(lowerBound: String?, upperBound: String?): VersionRange {
      return VersionRange(DelimitedVersion.parse(lowerBound), DelimitedVersion.parse(upperBound))
    }
  }
}
