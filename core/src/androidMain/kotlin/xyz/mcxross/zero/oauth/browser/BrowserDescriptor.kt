...
/**
 * Creates a description of a browser from the core properties that are frequently used to decide
 * whether a browser can be used for an authorization flow. In most cases, it is more convenient to
 * use the other variant of the constructor that consumes a [PackageInfo] object provided by the
 * package manager.
 *
 * @param packageName The Android package name of the browser.
 * @param signatureHashes The set of SHA-512, Base64 url safe encoded signatures for the app. This
 *   can be generated for a signature by calling [.generateSignatureHash].
...
(
  ...
  /**
   * The set of [signatures][android.content.pm.Signature] of the browser app, which have been
   * hashed with SHA-512, and Base-64 URL-safe encoded.
   */
   ...
   * app, which can be retrieved with the
   * [GET_SIGNATURES][android.content.pm.PackageManager.GET_SIGNATURES] flag when calling
  ...
  constructor(
    ...
    generateSignatureHashes(getSignatures(packageInfo)),
    packageInfo.versionName,
    useCustomTab
  )

  /**
   * Creates a copy of this browser descriptor, changing the intention to use it as a custom tab to
   * the specified value.
   */
  fun changeUseCustomTab(newUseCustomTabValue: Boolean): BrowserDescriptor {
    return BrowserDescriptor(packageName, signatureHashes, version, newUseCustomTabValue)
  }
    ...
    private fun getSignatures(packageInfo: PackageInfo): Array<Signature> {
      return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        ...
        if (packageInfo.signingInfo.hasMultipleSigners()) {
          ...
          // If the app has one signer, return an array containing just the one
          packageInfo.signingInfo.signingCertificateHistory
        }
      } else {
        // For API level 27 and below, the old method is used
