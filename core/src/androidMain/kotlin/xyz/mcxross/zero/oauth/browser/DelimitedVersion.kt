 ...
 * formats, treating any sequence of non-numeric characters as a delimiter, and discards these to
 * retain just the numeric content for comparison. Trailing zeroes in a version number are discarded
 * to produce a compact, canonical representation. Empty versions are equivalent to "0". Each
 * numeric part is expected to fit within a 64-bit integer.
 */
...
(private val mNumericParts: LongArray) : Comparable<DelimitedVersion> {
  override fun toString(): String {
    ...
    return buildString {
      append(mNumericParts[0])
      var index = 1
      while (index < mNumericParts.size) {
        append('.')
        append(mNumericParts[index])
        index++
      }
    }
  }
  ...
  companion object {
    ...
    fun parse(versionString: String?): DelimitedVersion {
      ...
      val stringParts =
        versionString.split("[^0-9]+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
